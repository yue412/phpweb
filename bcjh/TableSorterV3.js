...
TableSorter.prototype = {
    ...
    Sort : function(column)//执行排序.
    {
        ...
        for(var x = 1; x < this.Rows.length && SortAsNumber; x++)
        {
            SortAsNumber = this.IsNumeric(this.Rows[x].cells[column].innerText);
        }
        this.Rows.sort(
        function(row1, row2)
        {
            var result;
            var value1,value2;
            value1 = row1.cells[column].innerText;
            value2 = row2.cells[column].innerText;
            if(value1 == value2)
            {
                return 0;
            }
