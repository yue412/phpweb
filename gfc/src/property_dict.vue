    ...
    <div>
        <el-container>
            <div style="width: 15%" class="gfc-panel">
                <gfc-speciality-select
                    @speciality-change="load_data"
                ></gfc-speciality-select>
                <el-tree
                    id="element_type_tree"
                    ref="typeTree"
                    :data="element_types"
                    :props="tree_props"
                    :load="load_child"
                    @node-click="handleNodeClick"
                    node-key="element_type_id"
                    lazy
                ></el-tree>
            </div>
...
import GfcPropsTable from "./props_table.vue";
import GfcQtyTable from "./qty_table.vue";
...
export default {
    ...
    methods: {
        load_data(speciality_id) {
            ...
            do_ajax(
                "get_element_type.php?speciality_id=" +
                    speciality_id +
                    "&element_type_pid=-1",
                function (text) {
                    _this.element_types = JSON.parse(text);
                    var id =
                        _this.element_types.length > 0
                            ? _this.element_types[0].element_type_id
                            : -1;
                    _this.$nextTick(() => {
