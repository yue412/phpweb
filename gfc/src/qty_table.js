Vue.component('gfc-qty-table', {
    ...
    methods: {
        load_data() {
            var _this = this;
            do_ajax("../db/get_records.php?table_name=gfc_qty&element_type_id=" + this.elementTypeId, function (text) {
        ...
        add(qty) {
            ...
            this.handleConfirm = function () {
                ...
                do_ajax(encodeURI("../db/insert_record.php?" + build_url_params(arr)), function (text) {
                    ...
                    // ui添加
                    do_ajax("../db/get_records.php?" + build_url_params([['table_name', 'gfc_qty'], ['qty_id', id]]), function (text) {
        ...
        del(qty) {
            ...
            if (qty) {
                this.$confirm('确认删除？')
                    .then(_ => {
                        do_ajax("../db/del_record.php?table_name=gfc_qty&qty_id=" + qty.qty_id, function (text) {
                            if (text != "") {
                                _this.$message({
                                    message: text,
                                    type: 'error'
    ...
    template: '<div>\
        <el-button type="primary" @click="add(null)" size="medium" v-if="this.$root.can_edit">添加工程量</el-button>\
        ...
        <el-dialog title="编辑" :visible.sync="dialogVisible">\
            <el-form :model="edit_qty" label-width="120px">\
                ...
                <el-form-item label="数据类型" >\
                    <el-select v-model="edit_qty.data_type" placeholder="请选择">\
                        <el-option\
